...
/** Generates a random 3D vector that lies inside a unit disk. */
{
    while (true) {
        auto p = vec3(random_double(-1,1), random_double(-1,1), 0);
        // checks if the length squared of this vector is less than 1 to ensure the vector lies within the unit disk
        if (p.length_squared() < 1)
            return p;
    }
}
...
/** The reflection vector r is calculated using the formula: r=v−2⋅dot(v,n)⋅n
 * More on the graphical interpretation of the reflection vector calculation formula here:
 * https://medium.com/@tiffanychen1118/graphical-interpretation-of-the-reflection-vector-2d826236650d
 */
{
    return vector - 2*dot(vector,normal)*normal;
}

inline vec3 refract(const vec3& vector, const vec3& normal, double etai_over_etat)
/** The refracted vector is calculated using the formula: R′=R′⊥+R′∥ */
{
    auto cos_theta = std::fmin(dot(-vector, normal), 1.0);
    vec3 r_out_perp =  etai_over_etat * (vector + cos_theta*normal);
